...
# Lasso Regression for all count over both years and all seasons
...
Lscore = lasso.score(X_test, y_test) # Gives R^2
print('L-Score: ', Lscore)

# To get RMSE and MSE import sklearn metrics
...
plt.show()
plt.clf()
Lscore = lassoAll.score(X_test, y_test) # Gives R^2
...
# Now without temp
No_Temp = day_df[['season', 'mnth', 'holiday', 'weekday',
...
No_Wind = day_df[['season', 'mnth', 'holiday', 'weekday',
...
# Full model 
Full_df = day_df[['season','holiday', 'mnth', 'weekday',
       'workingday', 'weathersit', 'temp', 'hum', 'windspeed']]
X = Full_df
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.3  , random_state =42)
